...
void print_coor(double **PosIons, int natoms, float **boxcell, int n_atomtype, int *natoms_type, string *atomtype, int printtrj, int MDstep,char printmode, string filename)
{
	...
	if(printtrj==0)
	{
		...
		cout<<"****Printing coordinates file****"<<endl;
...
{
	cout<<"*****Coordinates of atoms in Angstrom****"<<endl<<endl;
...
{
	cout<<"*****Velocities of atoms in Angs/fs******"<<endl<<endl;
...
{
	cout<<"*****Forces on atoms in eV/Angs****"<<endl<<endl;
...
{
	cout<<"*****md.out file updated****"<<endl<<endl;  
	double Pot = 0;
	double KEner;
	float Temp;
	...
	KEner = CalKinEner(Vel, natoms, mass); 
	Temp = CalTemp(KEner, natoms);

	FILE *pFile; 
	if(MDstep==0)
	{
		pFile = fopen ("md.out", "w"); 	

		fprintf(pFile, "#MDstep\t Temp\t\t PE\t \tKE\t\t TotEner\n ");
	}
	else
	{
		pFile = fopen ("md.out", "a");

		fprintf(pFile, "%d\t %11.6f\t %11.6f\t %11.6f\t %11.6f \n", MDstep, Temp, Pot, KEner, Pot+KEner);
	}
